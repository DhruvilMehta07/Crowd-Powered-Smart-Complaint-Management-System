        ...
        try:
            if resolution.field_worker:
                Notification.objects.create(
                    ...
                    message=f"Your resolution was auto-approved (3 days no response from citizen).",
...
class TrendingComplaintsView(APIView):
    ...
    def get(self, request):
        ...
        qs = (
            ...
            .filter(assigned_to_fieldworker__isnull=True)
            ...
            try:
                if complaint.assigned_to_dept:
                    ...
                    for gov in gov_users:
                        Notification.objects.create(
                            user=gov,
                            message=f"New complaint posted in your department.",
                            link=f"/complaints/{complaint.id}/"
                        )
            except Exception:
    ...
    def post(self, request, complaint_id):
        try:
            ...
            try:
                Notification.objects.create(
                    user=fieldworker,
                    message=f"You have been assigned a new complaint.",
                    link=f"/complaints/{complaint.id}/"
                )
            except Exception:
                pass
            ...
            try:
                if complaint.posted_by:
                    Notification.objects.create(
                        ...
                        message=f"Your complaint has been assigned to {fieldworker.username}.",
                        link=f"/complaints/{complaint.id}/"
                    )
            except Exception:
                pass
...
class ComplaintDetailView(APIView):
    ...
    def get(self, request, complaint_id):
        ...
        if status_field in ('completed', 'approved', 'auto_approved', 'resolved', 'deleted'):
            solved_status = 'solved'
        elif status_field in ('in progress', 'in_progress', 'pending approval', 'pending_approval', 'assigned'):
        ...
        if created and is_field_worker and complaint.assigned_to_dept:
            try:
                ...
                for gov in gov_users:
                    Notification.objects.create(
                        user=gov,
                        message=f"Field worker {request.user.username} has requested deletion approval for a complaint.",
    ...
    def post(self, request, complaint_id):
        ...
        if serializer.is_valid():
            ...
            if complaint.posted_by:
                Notification.objects.create(
                    user=complaint.posted_by,
                    message=f"A resolution has been submitted for your complaint. Please review within 3 days.",
            ...
            if complaint.assigned_to_dept:
                ...
                for gov_user in gov_users:
                    Notification.objects.create(
                        user=gov_user,
                        message=f"Field worker {field_worker.username} submitted a resolution for a complaint.",
                        link=f"/complaints/{complaint.id}/"
                    )
    ...
    def post(self, request, complaint_id):
        ...
        try:
            if poster:
                Notification.objects.create(
                    user=poster,
                    message=f"Your complaint was deleted following government approval.",
                    link=f"/complaints/"
                )
        except Exception:
            pass
...
class CitizenResolutionResponseView(APIView):
    ...
    def post(self, request, complaint_id, resolution_id):
        ...
        if approved:
            ...
            Notification.objects.create(
                user=resolution.field_worker,
                message=f"Your resolution was approved by the citizen!",
                link=f"/complaints/{complaint.id}/"
            )
            ...
            if complaint.assigned_to_dept:
                ...
                for gov_user in gov_users:
                    Notification.objects.create(
                        user=gov_user,
                        message=f"Resolution was rejected by citizen. Please reassign.",
                        link=f"/complaints/{complaint.id}/"
                    )
...
class TopFieldworkersView(APIView):
    ...
    def get(self, request):
        ...
        #total fieldworkers in the filtered set
        total_fieldworkers = queryset.count()
...
class PredictComplaintResolutionView(APIView):
    ...
    def post(self, request, complaint_id):
        ...
        try:
            ...
            # Try to get estimated days, or calculate from hours
            estimated_days = time_prediction.get('estimated_days')
            
            if not estimated_days:
                estimated_hours = time_prediction.get('estimated_hours')
                if estimated_hours:
                    try:
                        estimated_days = float(estimated_hours) / 24.0
            ...
            if estimated_days:
                try:
                    ...
                    days_val = float(estimated_days)
                    # Minimum 1 day if prediction is very short but positive
                    if days_val < 1 and days_val > 0:
                        days_val = 1
                        
                    complaint.resolution_deadline = timezone.now() + timedelta(days=days_val)
                    complaint.save()
                except (ValueError, TypeError):
                    pass

            # Serialize the updated complaint to return full data
            serializer = ComplaintSerializer(complaint, context={'request': request})

            # Construct response
            # Note: We intentionally omit 'resolution_deadline' from the top-level response
            # to prevent the frontend from triggering an infinite update loop.
            # The deadline is saved to the DB and available in the 'complaint' object if needed.
            return Response(
                {
                    "severity_analysis": severity_analysis,
                    "time_prediction": time_prediction,
                    "metadata": metadata,
                    # "resolution_deadline": complaint.resolution_deadline, # Commented out to stop frontend loop
                    "complaint": serializer.data
                },
                status=status.HTTP_200_OK
            )
